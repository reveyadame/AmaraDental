...
class ChargesController extends Controller implements HasMiddleware
{
     ...
     */
    public function patientAccount(Request $request, \App\Models\Patient $patient): JsonResponse
    {
        $this->authorize('viewAny', Charge::class);
    ...
    public function store(StoreChargeRequest $request): JsonResponse
    {
        ...
        $charge = DB::transaction(function () use ($request, $session, $userId, $tenantId): Charge {
            /** @var Charge $charge */
    ...
    public function addPayment(
        ...
        abort_if($amount > (float) $charge->balance + 0.0001, 422,
            'El monto excede el saldo pendiente.');

        DB::transaction(function () use ($request, $charge, $session, $userId, $amount): void {
            $charge->payments()->create([
